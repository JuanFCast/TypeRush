      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        {items.length > 0 && (
          <button
            type="button"
            onClick={onClear}
            className="rounded-lg border border-line px-2.5 py-1 text-xs font-semibold text-muted transition active:scale-[0.98]"
          >
            Limpiar
          </button>
        )}
      ...
      {items.length === 0 ? (
        ...
        <ul className="flex flex-col gap-2.5">
          {items.map((it) => (
            <li
              key={it.id}
              className="rounded-2xl border border-line bg-surface2 p-3.5"
            >
